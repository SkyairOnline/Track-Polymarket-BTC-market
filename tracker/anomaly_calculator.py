...
class AnomalyCalculator:
    ...
    def check(self, up_ask: float, down_ask: float) -> List[AnomalyEvent]:
        ...
        Guard: only count if combined best ask < 1.07 (market is liquid/fair).
        When combined >= 1.07 the spread is too wide — the crossing is noise, not signal.
        """
        if up_ask + down_ask >= 1.07:
            return []
