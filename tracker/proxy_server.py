"""
Binance proxy — runs on Railway alongside the tracker.

GET  /proxy/klines   → api.binance.com/api/v3/klines  (pass-through query params)
            ...
            async with s.ws_connect(_BINANCE_WS) as binance:
                ...
                try:
                    async for _ in browser:
                        pass  # browser→binance not needed for aggTrade read-only stream
                finally:
                    task.cancel()
